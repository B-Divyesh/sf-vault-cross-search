...
## Installed desktop sample vault

On a fresh desktop-app session, **Load sample project** opens `src-tauri/resources/vault-cross-search-sample.kdbx`, a fake bundled vault containing Acme VPN and Acme status metadata. The native core reads the bundled bytes directly into the same session-only metadata index used for selected vaults; it does not invoke a file picker or write a sample file. The first-run walkthrough in the app covers load, search, and Lock all. Lock all clears the sample exactly as it clears a selected vault. The browser demo remains a separate web sandbox and is not used to exercise this desktop path.
